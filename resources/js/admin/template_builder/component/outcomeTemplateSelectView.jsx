...
import { apiLearningCompTempPut } from '../../../api'

const OutcomeTemplateSelectView = (props) => {
    ...
    React.useEffect(()=>{
        var temp = [];
        // console.log(typeof props.designtype_outcomes);
        if(typeof props.designtype_outcomes != "undefined"){
            var x = Object.values(props.designtype_outcomes);
            setOutcomes(x);
        }
       
    }, [props.designtype_outcomes])
    React.useEffect(()=>{
        setLOcheck(initCheck());
    }, [outcomes])
    ...
    const handleOnCheck = (event) => {
        ...
        if( lo[event.target.name]["outcome_id"]  ==  lo[event.target.name]["ulo_id"]){
            ...
            if(event.target.checked){
                ...
                outcomes.find(x => x.id == event.target.name).slo_outcome.map((_slo) => 
                    {
    ...
    const handleOnSave = () => {
        ...
        if(typeof props.onFinishSelect != "undefined"){
            props.onFinishSelect(request);
        }
    }
...
export default OutcomeTemplateSelectView;