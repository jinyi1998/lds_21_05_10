...
import { apiFeedbackList, apiFeedbackGet, apiFeedbackPost, apiFeedbackPut} from '../../../api';

const FeedbackOptsController = () => {
    ...
    const onOpenEditDialog = (id) => {
        ...
        if(id > 0){
            apiFeedbackGet({"id":id}).then((response) => {
    ...
    const onSaveOpt = () => {
        if(editOpt.id == -1){
            apiFeedbackPost(editOpt).then(response => {
                reloadTaskTypeOpts();
                setIsEditOpen(false)
            })
        }else{
            apiFeedbackPut(editOpt).then(response => {
                reloadTaskTypeOpts();
                setIsEditOpen(false)
            })
        }
    ...
    const onDragEnd = (result) => {
        ...
        var temp =  JSON.parse( JSON.stringify(feedback) );
        var source =  JSON.parse( JSON.stringify(feedback[result.source.index]) );
        ...
        var updates = [];
        temp.map(_feedback => {
            updates.push(apiFeedbackPut(_feedback));
        });
    ...
    return (
        <React.Fragment>
                                            ...
                                            <RootRef rootRef={provided.innerRef}>
                                                  <TableBody style={getListStyle(snapshot.isDraggingOver)}>
                                                        ...
                                                        .map((_feedback, index ) => (
                                                                ...
                                                                <Draggable key={index} draggableId={_feedback.id.toString()} index={index} isDragDisabled = {!enableDrag}>
                                                                {(provided, snapshot) => (
                                                                    <TableRow key = {_feedback.id} hover {...getDraggable(provided, snapshot)}>
                                                                        {
                                                                            enableDrag?
                                                                        ...
                                                                        <TableCell component="th" scope="row"  onClick={ () => onOpenEditDialog(_feedback.id) }>
                                                                            
                                                                            <ListItemText 
                                                                                primary = {_feedback.name}
                                                                                secondary= {
                                                                                    "Updated By:" + _feedback.updatedby.name + "@" + _feedback.updated_at
                                                                                    + " || " 
                                                                                    + "Created By: " + _feedback.createdby.name + "@" + _feedback.created_at } 
                                                                            />
                                                                        </TableCell>
                                                                        <TableCell component="th">
                                                                            <IconButton color="primary" onClick = {() => {event.preventDefault(); onOpenEditDialog(_feedback.id);}}>
                                                                                <EditAttributesIcon />
                                                                            </IconButton>
                                                                        </TableCell>
                                                                    </TableRow>
            ...
            <Dialog onClose={onCloseEditDialog} open={isEditOpen}  maxWidth = "md">
                <DialogTitle> {editOpt.id == -1? "Add Feedback Option " : "Edit Feedback Option"}</DialogTitle>
...
export default FeedbackOptsController;